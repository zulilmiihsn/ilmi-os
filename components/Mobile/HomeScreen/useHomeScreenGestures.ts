...
export function useHomeScreenGestures({
    ...
    const triggerCloseApp = useCallback(() => {
        ...
        container.style.transition = 'transform 0.35s cubic-bezier(0.32, 0.72, 0, 1), opacity 0.25s ease';
    ...
    const handleBottomPointerMove = useCallback((e: React.PointerEvent) => {
        ...
        if (deltaY > 0) {
            ...
            appContainerRef.current.style.opacity = progress > 70
                ? String(Math.max(0, 1 - (progress - 70) / 30))
                : '1';
        }
    }, [currentApp, appToOpen, appContainerRef]);

    const handleBottomPointerUp = useCallback((e: React.PointerEvent) => {
        ...
        if (deltaY > 25) {
            ...
            const container = appContainerRef.current;
            container.style.transition = 'transform 0.3s cubic-bezier(0.32, 0.72, 0, 1), opacity 0.3s ease';
        ...
        if (touch.isTopSwipe) {
            if (touch.isTopRight) {
                ...
                if (isDeliberateSwipe) {
                    triggerHaptic('light');
                    if (panel) {
                        panel.style.transition = 'transform 0.35s cubic-bezier(0.32, 0.72, 0, 1), opacity 0.25s ease';
                    ...
                    if (panel) {
                        panel.style.transition = 'transform 0.3s cubic-bezier(0.32, 0.72, 0, 1), opacity 0.25s ease';
