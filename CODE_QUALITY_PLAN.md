...
- [x] Jadikan temuan statis UI sebagai skenario reproduksi desktop/mobile sebelum mengklaim gejalanya terjadi di browser. Tercatat 2026-09-23: seluruh klaim UI Tahap 4–5 dibuktikan via `browser-tests/` (shell/desktop, mobile, focus, lifecycle, theme, filesync, theme-visual) — termasuk bukti sebelum/sesudah (filesync gagal-sebelum/lolos-sesudah); klaim tanpa skenario tidak dihitung (contoh dikoreksi: kamera/mic, touch fisik, throttling, screen reader — diverifikasi pemilik di perangkat nyata 2026-09-23).
- [x] Catat data lama yang harus dipertahankan serta batas fitur demo yang belum diputuskan untuk diperluas. Tercatat 2026-09-23 — kunci lama yang dipertahankan: `notes`→`ilmi:notes:v1` (sumber dipertahankan hingga tulis terverifikasi), `fileSystem`→`ilmi_file_system` (legacy dihapus hanya setelah merge tersimpan), wallpaper lama→`.webp` + skema settings v1 (migrasi `migrateSettingsState`), `ilmi_mail_data_v1` (fallback seeds, tanpa migrasi), `camera_photos` (daftar lokal, tanpa migrasi). Batas demo yang dipertahankan (perlu keputusan produk sebelum diperluas): Safari/Messages/Music/Phone placeholder (layar Under Development), Mail mock interaktif lokal, Maps explorer mock, Photos 12 mock Unsplash + foto lokal, Spotlight Top Hit statis, ekspresi font Calculator.
...
Terverifikasi 2026-09-23 pada HEAD `c153e5f`: type-check, lint, 55 unit test, production build, dan 20/20 browser test (termasuk focus-return/Escape/Tab-trap keyboard) lolos; diff `c153e5f` hanya menyentuh 5 file yang direncanakan (folder rename, theme test, catatan strictness); risiko sisa dan item opsional tercatat di Tahap 6-8 dan Di Luar Cakupan. Tambahan 2026-09-23 (live-sync Finder/Terminal): type-check, lint, 59 unit test (10 file), production build, dan 21/21 browser test lolos pada working tree ini. Tambahan 2026-09-23 (visual vs OS + `pb-safe`): type-check, lint, 59 unit test, production build, dan 22/22 browser test lolos. Tambahan 2026-09-23 (`noUncheckedIndexedAccess` ON): type-check ketat, lint, 59 unit test, production build, dan 22/22 browser test lolos. Tambahan 2026-09-23 (coverage): type-check ketat, lint, 112 unit test (12 file, coverage 86% stmts/87% branch/83% funcs/86% lines), production build, dan 22/22 browser test lolos. Tambahan 2026-09-23 (toolchain: Next 15.5.9 + ESLint 10.11.0): type-check ketat, lint nol warning, 112 unit test, production build 15.5.9, dan 22/22 browser test lolos (satu visual flaky pada run pertama, hijau pada dua run ulang). Tambahan 2026-09-23 (frame Control Center desktop): panel + tile tak pernah punya style (kelas `control-*` tak terdefinisi sejak awal) sehingga tidak ada kaca blur; diperbaiki mengikuti macOS + sibling mobile (panel `bg-black/30 backdrop-blur-2xl`, tile `bg-white/10`, aktif `bg-[#007aff]`) — type-check ketat, lint nol warning, 112 unit test, production build, dan 23/23 browser test lolos. Tambahan 2026-09-23 (animasi restore + fade menu): Show All kini memutar balik trip minimize (render-phase edge agar first paint sudah kecil, StrictMode-aman), dropdown menubar fade/zoom 100ms + `role="menu"` — type-check ketat, lint nol warning, 112 unit test, production build, dan 24/24 browser test lolos (visual finder flaky sekali, hijau saat run ulang). Tambahan 2026-09-23 (dock tooltip + launch bounce): nama app di atas ikon saat hover (CSS murni, `delay-300` ala macOS, `aria-hidden`) + ikon memantul dua kali saat launch (`dockBounce` 0.6s, nonaktif saat `prefers-reduced-motion`) — type-check ketat, lint nol warning, 112 unit test, production build, dan 25/25 browser test lolos (tes visual finder dipindah-parkir mouse-nya agar tooltip tak bocor ke baseline resting-state). Tambahan 2026-09-23 (tile CC: hover/active/focus + AirDrop): tile `hover:brightness-125 active:scale-95` + outline fokus; tile User Profile yang mati diganti toggle AirDrop (tile ke-4 modul konektivitas macOS) — type-check ketat, lint nol warning, 116 unit test, production build, dan 28/28 browser test lolos sekali jalan. Tambahan 2026-09-24 (audit + fidelity iOS): dots halaman dipasang (komponen ada tapi tak dipakai), long-press area kosong masuk edit + tap kosong keluar (pointer events, toleran device), close app zoom balik ke ikon, folder buka/tutup zoom dari ikon (skala rem-aware); badge hapus −/× sengaja dilewatkan (tanpa App Library = jebakan) — type-check ketat, lint nol warning, 116 unit test, production build, dan 32/32 browser test lolos. Tambahan 2026-09-23 (ikon tile CC seragam + rune BT dibalance): glyph FA campur solid/brands yang bobot optiknya beda diganti SVG inline seragam (stroke 2px round, pola tab Clock; bulan sabit filled ala SF); rune bluetooth yang sempit dibesarkan `scale-[1.2]` — type-check ketat, lint nol warning, 116 unit test, production build, dan 28/28 browser test lolos. Tambahan 2026-09-23 (ikon tile CC seragam): glyph FA campur solid/brands yang bobot optiknya beda diganti SVG inline seragam (stroke 2px round, pola tab Clock; bulan sabit filled ala SF) — type-check ketat, lint nol warning, 116 unit test, production build, dan 28/28 browser test lolos. Tambahan 2026-09-23 (tile CC: hover/active/focus): tile kini `hover:brightness-125 active:scale-95` + outline fokus keyboard; font/ikon sudah sesuai konvensi (text-xs label, glyph text-xl, FA sebagai pengganti SF) — type-check ketat, lint nol warning, 116 unit test, production build, dan 28/28 browser test lolos sekali jalan. Tambahan 2026-09-23 (Exposé-lite): F3 menyebar semua window terbuka (termasuk yang minimize) ke tile grid via `layoutExposeTiles` murni + teruji; klik/Enter memilih (fokus + unminimize dengan trip restore), Esc/klik-dim/F3 keluar tanpa perubahan, auto-keluar saat nol window, focus-return — type-check ketat, lint nol warning, 116 unit test, production build, dan 27/27 browser test lolos (visual finder flaky sekali, hijau pada run ulang penuh). Tambahan 2026-09-24 (dots ketutup dock): pagination `bottom-98px` vs dock menutup 104px — naik ke `bottom-112px`, baseline mobile-home diregenerasi (sengaja, 228 byte beda) — type-check ketat, lint nol warning, 116 unit test, production build, dan 32/32 browser test lolos. Tambahan 2026-09-23 (tile CC bulat ala Apple): referensi visual Apple (Wikipedia Control Center) memastikan tile konektivitas berbentuk lingkaran, bukan kotak — 4 tile desktop diganti `rounded-xl`→`rounded-full`, glyph bluetooth resmi FA dipertahankan — type-check ketat, lint nol warning, 116 unit test, production build, dan 28/28 browser test lolos. Tambahan 2026-09-23 (rune bluetooth resmi FA): gambar tangan rune tak mirip logo umum (terbaca bowtie/play); diganti glyph resmi `fa-bluetooth` (bukan varian -b) ukuran `text-lg` agar bobotnya sejajar — type-check ketat, lint nol warning, 116 unit test, production build, dan 28/28 browser test lolos. Tambahan 2026-09-23 (rune bluetooth digambar ulang): path Lucide terbaca seperti bowtie/play lalu stem + chevron seperti tombol play; digambar ulang eksplisit (stem + dua busur segitiga, miter join tajam) — type-check ketat, lint nol warning, 116 unit test, production build, dan 28/28 browser test lolos.
